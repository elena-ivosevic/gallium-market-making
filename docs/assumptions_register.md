...
demo seed produces terminal wealth of **~$35,600** and mark-to-market P&L of
**~$48,400** — in the same order of magnitude as Phase 1/2's instant-restock
baseline (~$85,000–135,000 depending on mode), not a different universe.
Terminal wealth is somewhat lower than Phase 1/2's, primarily because the
reorder-point formula conservatively assumes 100% of generated demand is
filled (the real fill rate is ~20%), which causes some inventory
over-accumulation (more cash tied up in stock than strictly necessary) — a
known, flagged consequence of that conservative choice, not a new bug.

A stressed scenario (reliability dropped to 50%) produces a clearly negative
result (~-$427,000 to -$440,000 mark-to-market/terminal wealth, ~1,350kg
lost to failed deliveries out of ~2,700kg ordered) — the kind of directional
sanity check this project leans on in place of a backtest: worse supply
reliability should make the dealer worse off, and it now visibly does, by a
plausible magnitude tied directly to the kg actually lost.
